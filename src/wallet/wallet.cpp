        ...
        int blockHeight, bool fPrecompute)
{
    ...
    if (GetBoolArg("-ulgstake", true) && !fPrecompute) {
    ...
    /* Disable zULG Staking
    //zULG
    if ((GetBoolArg("-zulgstake", true) || fPrecompute) && chainActive.Height() > Params().Zerocoin_Block_V2_Start() && !IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ...
        std::set<CMintMeta> setMints = zulgTracker->ListMints(true, true, fUpdate);
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 ULG collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<std::pair<CScript, CAmount> > vecSend;
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 ULG collateral
        ...
        )
{
    ...
    for (std::unique_ptr<CStakeInput>& stakeInput : listInputs) {
        ...
        if (Stake(pindexPrev, stakeInput.get(), nBits, nTxNewTime, hashProofOfStake)) {
            ...
            {
                TRY_LOCK(zulgTracker->cs_spendcache, fLocked);
                if (!fLocked)
                    continue;
                         ...
                         CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    while (nLockAttempts < 100) {
        TRY_LOCK(zulgTracker->cs_spendcache, lockSpendcache);
        if (!lockSpendcache) {
        ...
        for (auto &it : mapMintsSelected) {
            ...
            CMintMeta meta = zulgTracker->Get(GetSerialHash(mint.GetSerialNumber()));
            CoinWitnessData *coinWitness = zulgTracker->GetSpendCache(meta.hashStake);
            ...
            if (!GenerateAccumulatorWitness(coinWitness, mapAccumulators, pindexCheckpoint)) {
                receipt.SetStatus(_("Couldn't generate the accumulator witness"),
                                  ZULG_FAILED_ACCUMULATOR_INITIALIZATION);
            ...
            try {
                ...
                if (!CheckCoinSpend(spend, accumulator, receipt)) {
                    receipt.SetStatus(_("CoinSpend: failed check"), ZULG_SPEND_ERROR);
                    return error("%s : %s", __func__, receipt.GetStatusMessage());
                }
    ...
    if (nLockAttempts == 100) {
        ...
        receipt.SetStatus(_("could not get lock on cs_spendcache"), ZULG_TXMINT_GENERAL);
        return false;
    }
                                    ...
                                    CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    while (nLockAttempts < 100) {
        ...
        for (auto &it : mapMintsSelected) {
            ...
            CTxIn in;
            if(!ZULGModule::createInput(in, mint, hashTxOut)){
        ...
        bool isPublicSpend)
{
    ...
    const int nMaxSpends = Params().Zerocoin_MaxPublicSpendsPerTransaction(); // Maximum possible spends for one zULG public spend transaction
...
std::string CWallet::GetUniqueWalletBackupName(bool fzulgAuto) const
{
...
void CWallet::ZUlgBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (boost::filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                boost::filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozulgbackup-%d.dat", i + 1);
    ...
    if(!GetArg("-zulgbackuppath", "").empty()) {
        boost::filesystem::path customPath(GetArg("-zulgbackuppath", ""));
...
{
    // We don't even need to worry about this code.. no zULG.
    /*
    LogPrintf("Precomputer started\n");
    RenameThread("ultragate-precomputer");
    ...
    while (true) {
        ...
        for (std::unique_ptr <CStakeInput>& stakeInput : listInputs) {
...

            {
                ...
                CoinWitnessData* witnessData = zulgTracker->GetSpendCache(serialHash);
