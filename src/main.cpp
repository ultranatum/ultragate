...
/** Fees smaller than this (in uulg) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zULG spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zULG without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zULG spend tx %s already in block %d",
            ...
            // Check that zULG mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight == 0) {
        ...
        nSubsidy = 0.1 * COIN;
    } else if (nHeight < 38000 && nHeight >= 28000) {
        nSubsidy = 4 * COIN;
	} else if (nHeight < 43000 && nHeight >= 38000) {
        nSubsidy = 6 * COIN;
	} else if (nHeight < 46000 && nHeight >= 43000) {
        nSubsidy = 7 * COIN;
	} else if (nHeight < 48000 && nHeight >= 46000) {
        nSubsidy = 1 * COIN;
	} else if (nHeight < 50000 && nHeight >= 48000) {
        nSubsidy = 0.5 * COIN;
	} else if (nHeight < 56000 && nHeight >= 50000) {
        nSubsidy = 13 * COIN;
	} else if (nHeight < 63000 && nHeight >= 56000) {
        nSubsidy = 16 * COIN;
	} else if (nHeight < 65000 && nHeight >= 63000) {
        nSubsidy = 10 * COIN;
	} else if (nHeight < 68000 && nHeight >= 65000) {
        nSubsidy = 2 * COIN;
	} else if (nHeight < 70000 && nHeight >= 68000) {
        nSubsidy = 1 * COIN;
	} else if (nHeight < 78000 && nHeight >= 70000) {
        nSubsidy = 25 * COIN;
	} else if (nHeight < 88000 && nHeight >= 78000) {
        nSubsidy = 33 * COIN;
	} else if (nHeight < 93000 && nHeight >= 88000) {
        nSubsidy = 34 * COIN;
	} else if (nHeight < 98000 && nHeight >= 93000) {
        nSubsidy = 50 * COIN;
	} else if (nHeight < 100000 && nHeight >= 98000) {
        nSubsidy = 2 * COIN;
	} else if (nHeight < 103000 && nHeight >= 100000) {
        nSubsidy = 10 * COIN;
	} else if (nHeight < 113000 && nHeight >= 103000) {
        nSubsidy = 50 * COIN;
	} else if (nHeight < 118000 && nHeight >= 113000) {
        nSubsidy = 100 * COIN;
	} else if (nHeight < 123000 && nHeight >= 118000) {
        nSubsidy = 150 * COIN;
	} else if (nHeight < 128000 && nHeight >= 123000) {
        nSubsidy = 200 * COIN;
	} else if (nHeight < 133000 && nHeight >= 128000) {
        nSubsidy = 150 * COIN;
	} else if (nHeight < 138000 && nHeight >= 133000) {
        nSubsidy = 100 * COIN;
	} else if (nHeight < 143000 && nHeight >= 138000) {
        nSubsidy = 50 * COIN;
	} else if (nHeight < 153000 && nHeight >= 143000) {
        nSubsidy = 30 * COIN;
	} else if (nHeight < 183000 && nHeight >= 153000) {
        nSubsidy = 15 * COIN;
	} else if (nHeight < 200000 && nHeight >= 183000) {
        nSubsidy = 10 * COIN;
	} else {
        nSubsidy = 5 * COIN;
    }
...
{
    RenameThread("ultragate-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZULGMinted()
{
...
bool RecalculateULGSupply(int nHeightStart)
{
...
bool UpdateZULGSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateULGSupply(Params().Zerocoin_StartHeight());
    }

    //Track zULG money supply in the block index
    if (!UpdateZULGSupply(block, pindex))
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zUlgSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert ULG to zULG
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zULG stake block is less than allowed start height", __func__);

    if (CZUlgStake* zULG = dynamic_cast<CZUlgStake*>(stake)) {
        CBlockIndex* pindexFrom = zULG->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zULG stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zULG stake does not have required confirmation depth", __func__);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zULG->GetValue()));
        if (nChecksum200 != zULG->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zULG->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZULG() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zULG fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zULG mints and %d zULG spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Ultragate: We use certain sporks during IBD, so check to see if they are
...
int ActiveProtocol()
{
    // SPORK_14 is used for 70931 (v1.0.0.1)
    if (IsSporkActive(SPORK_14_NEW_PROTOCOL_ENFORCEMENT))
            return MIN_PEER_PROTO_VERSION_AFTER_ENFORCEMENT;

    // SPORK_15 was used for 70932 , commented out now.
