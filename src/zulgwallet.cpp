...
CzULGWallet::CzULGWallet(std::string strWalletFile)
{
...
void CzULGWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zulgTracker->HasPubcoinHash(pMint.first)) {
...
bool CzULGWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    pwalletMain->zulgTracker->Add(dMint, true);
...
void CzULGWallet::GenerateDeterministicZULG(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzULGWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
