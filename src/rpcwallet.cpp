...
UniValue movecmd(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw runtime_error(
            ...
            "\nMove 0.01 ULG from timotei to akiko with a comment and funds have 6 confirmations\n" +
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const string& name_, keys) {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid UltraGate address: ")+name_);
...
/* ulg UniValue getzerocoinbalance(const UniValue& params, bool fHelp)
{
...
UniValue listmintedzerocoins(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    set<CMintMeta> setMints = pwalletMain->zulgTracker->ListMints(true, false, true);
...
UniValue listzerocoinamounts(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    set<CMintMeta> setMints = pwalletMain->zulgTracker->ListMints(true, true, true);
...
UniValue resetspentzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzULGTracker* zulgTracker = pwalletMain->zulgTracker.get();
    set<CMintMeta> setMints = zulgTracker->ListMints(false, false, false);
...
UniValue exportzerocoins(const UniValue& params, bool fHelp)
{
    ...
    CzULGTracker* zulgTracker = pwalletMain->zulgTracker.get();
    set<CMintMeta> setMints = zulgTracker->ListMints(!fIncludeSpent, false, false);
