...
#include "ui_zulgcontroldialog.h"
...
std::set<std::string> ZUlgControlDialog::setSelectedMints;
std::set<CMintMeta> ZUlgControlDialog::setMints;

bool CZUlgControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZUlgControlDialog::COLUMN_DENOMINATION || column == ZUlgControlDialog::COLUMN_VERSION || column == ZUlgControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZUlgControlDialog::ZUlgControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZUlgControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZUlgControlDialog::~ZUlgControlDialog()
{
    delete ui;
}

void ZUlgControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZUlgControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZUlgControlWidgetItem* itemDenom(new CZUlgControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZUlgControlWidgetItem *itemMint = new CZUlgControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zulgTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zulgTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zULG.";
            else if (!mint.isSeedCorrect)
                strReason = "The zULG seed used to mint this zULG is not the same as currently hold in the wallet";
            else
...
void ZUlgControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZUlgControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZUlg_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZUlgControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZUlgControlDialog::GetSelectedMints()
{
...
void ZUlgControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
