...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 ULG.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyzULGReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzULGReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzULGReset.connect(boost::bind(NotifyzULGReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzULGReset.disconnect(boost::bind(NotifyzULGReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zulgTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
