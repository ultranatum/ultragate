...
CzULGTracker::CzULGTracker(std::string strWalletFile)
{
...
CoinWitnessData* CzULGTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzULGTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
{
    bool iszULGWalletInitialized = (NULL != zULGWallet);
    CMintMeta meta;
    ...
    if (! iszULGWalletInitialized)
        zULGWallet = new CzULGWallet(strWalletFile);
...
std::set<CMintMeta> CzULGTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    ...
    if (fUpdateStatus) {
        ...
        CzULGWallet* zULGWallet = new CzULGWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zULGWallet);
        }
        delete zULGWallet;
        LogPrint("zero", "%s: added %d dzulg from DB\n", __func__, listDeterministicDB.size());
    }
