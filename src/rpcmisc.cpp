...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total ultragate balance of the wallet\n"
    ...
    /* ulg UniValue zulgObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zulgObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
