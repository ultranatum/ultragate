...
extern UniValue DoZulgSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzulgseed(const UniValue& params, bool fHelp);
extern UniValue getzulgseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzulg(const UniValue& params, bool fHelp);
extern UniValue dzulgstate(const UniValue& params, bool fHelp);
