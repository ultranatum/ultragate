...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    UniValue zulgObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zulgObj.push_back(Pair(std::to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zulgObj.push_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
    result.push_back(Pair("zULGsupply", zulgObj));
    ...
    if (block.IsProofOfStake()) {
        ...
        stakeData.push_back(Pair("stakeModifierHeight", ((stake->IsZULG()) ? "Not available" : std::to_string(
...
UniValue getchecksumblock(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
            ...
            "  \"zULGsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zULG denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zULG denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zULG denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zULG denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zULG denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zULG denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zULG denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zULG denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zULG denominations\n"
            "  }\n"
            "}\n"
...
UniValue getaccumulatorwitness(const UniValue& params, bool fHelp)
{
    ...
    if (!GenerateAccumulatorWitness(pubCoin, accumulator, witness, nMintsAdded, strFailReason)) {
        receipt.SetStatus(_(strFailReason.c_str()), ZULG_FAILED_ACCUMULATOR_INITIALIZATION);
