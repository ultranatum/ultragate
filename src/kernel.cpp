...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZUlgStake(spend));
    } else {
        ...
        CUlgStake* ulgInput = new CUlgStake();
        ulgInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(ulgInput);
    }
